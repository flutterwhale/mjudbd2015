...
<body>
	<nav class="navbar navbar-default">
	        <div class="container-fluid">
	          <div class="navbar-header">
	               ...
	               <a class="navbar-brand" href="${pageContext.request.contextPath}/LoginController/logout.do">GroupwareSystem</a>
	          </div>
	...
	<form class="form-horizontal" onSubmit="return check()"
			...
			<fieldset>
		         <legend><label class="col-sm-3 control-label">개인정보(개발자 수정)</label></legend>
			...
			<div class="form-group">
	         ...
	         <div class="form-group">
	          ...
	          <div class="form-group">
	           	<label for="inputname" class="col-sm-4 control-label">KOSA기술등급</label>
	            <div class="col-sm-2">
			    	<label class="radio-inline"><input class="radio-inline" type="radio" value="10"name="tech_level"  <% if(showBean.getTech_level()==10){  %>checked <%} %>> 고급</label> 
		        </div>
		        <div class="col-sm-2">
			        <label class="radio-inline"><input class="radio-inline" type="radio" value="20" name="tech_level"  <% if(showBean.getTech_level()==20){  %>checked <%} %>>중급</label> 
		        </div>
		        <div class="col-sm-2">
			        <label class="radio-inline"><input class="radio-inline" type="radio" value="30" name="tech_level"  <% if(showBean.getTech_level()==30){  %>checked <%} %>>초급</label>
		        </div>
	         </div>
	         <div class="form-group"> 
		     	<label for="inputname" class="col-sm-4 control-label">프로그래밍언어</label>
		     	<div class="col-sm-5"> 
			         ...
			         <tbody>
						...
						int i;
				         ...
				         <tr id ="item<%=i+1 %>" class="item<%=i+1 %>">
							...
							<td>
								<select id="select<%=i+1 %>" name="select<%=i+1 %>" class="form-control"  onclick="langugeEtc(this.value,this.name)" >
				                	...
				                	<option value="<%=language_list.get(i)%>" selected ><%=language_list.get(i)%></option>
				                	<option value="etc">etc</option>
				                </select>
				                <input class="form-control" type="hidden" id="language<%=i+1 %>" name="language<%=i+1 %>" value="<%=language_list.get(i)%>"  onfocusout="languageEtcInput(this.name,this.value)" size="8"/>
				                <label class="col-sm-3"><input class="radio-inline" type="radio" value="10" name="language_level<%=i+1 %>"  <% if((Integer)language_level_list.get(i)==30){  %>checked <%} %>>상  </label> 
								<label class="col-sm-3"><input class="radio-inline" type="radio" value="20" name="language_level<%=i+1 %>" <% if((Integer)language_level_list.get(i)==20){  %>checked <%} %> >중  </label>
								<label class="col-sm-3"><input class="radio-inline" type="radio" value="30" name="language_level<%=i+1 %>"<% if((Integer)language_level_list.get(i)==10){  %>checked <%} %> >하  </label>
							</td>
							<td><button <%if(i==0){ %>class="btn btn-info"<%} else{ %> class="delBtn btn btn-info" <% }%> name="language_del<%=i+1 %>" value="<%=i+1 %>" <%if(i==0){ %>onclick="alert('첫 행은 삭제가 불가능합니다.'); return false<%} else{ %> onclick="return false;" <% }%> ">삭제</button></td>
						</tr>
					<%} %>
					</tbody>
				</table>
