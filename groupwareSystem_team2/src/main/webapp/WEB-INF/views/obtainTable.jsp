	...
	<div class="obtain_table_wrapper" style="margin: 10px 40px;">
		<div class="obtain">
			...
			<table class="table table-striped table-hover" border="1">
				...
				<tr align="center">
					<td><a
						href='${pageContext.request.contextPath}/ProjectController/showObtainInformation?oid=<%=c.getObtain_Order_Identifier() %>'><%=c.getObtain_Order_Identifier() %></a></td>
					<td><a
						href='${pageContext.request.contextPath}/ProjectController/showObtainInformation?oid=<%=c.getObtain_Order_Identifier() %>'><%=c.getObtain_Name() %></td>
					<td><%=c.getStart_Date() %></td>
					<td><%=c.getEnd_Date() %></td>
					<td><input type="text" maxlength="70px"
						value="<%=c.getComment()%>" readonly="readonly"></td>
					<td><%=c.getOrder_Company()%></td>
					<td><%=c.getLocation()%></td>
					<td><a
						href='${pageContext.request.contextPath}/LoginController/retrieveUser?id=<%=c.getWriter_User()%>'><%=c.getWriter_User()%></a><br /><%=c.getWriter_name() %></td>
					...
					%>
						<button type="button" class="btn btn-success"
							...
							%> <a
						href='${pageContext.request.contextPath}/LoginController/retrieveUser?id=<%= c.getPM() %>'><%= c.getPM() %></a>
						<% } %>
					</td>
					<%}%>
				</tr>
		...
		<div class="main_button">
			...
			<button type="button" class="btn btn-primary"
				onclick="location.href='${pageContext.request.contextPath}/ProjectController/newObtainPage'">제안
				올리기</button>
